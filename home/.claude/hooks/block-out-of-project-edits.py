...
repo root but outside the worktree). That is the specific footgun where edits
...
no-op, and edits to sibling repos, multi-repo trees, or any other path proceed
untouched. Also fails open when the payload can't be parsed or CLAUDE_PROJECT_DIR
is unset.
"""
...
def main() -> int:
    ...
    # Only the parent checkout is off-limits: under the repo root, outside the worktree.
    if not (_under(abs_path, parent_repo) and not _under(abs_path, proj)):
        return 0
