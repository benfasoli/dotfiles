...
- Avoid two things: manager/corporate idioms ("heads-down," "drove the call," "punt," "circle back") and LLM fluff. Kill-list: leverage, utilize, enabling/allowing/ensuring as padding, robust, seamless, comprehensive, significant, successfully, streamlined, facilitate, various, "a number of," "in order to," "plays a key role," "aimed at," "as part of," and vague adjectives (powerful, efficient) with no number behind them. Also cut meta-commentary that grades the work itself — load-bearing, critical, key, crucial, notably, genuinely, "the real fix," "the real bug," "the important part." Cut verification narration — "Verified via QA," "reviewed with X," lint/test claims; the post asserts outcomes. And do not narrate discovery — "testing surfaced the bug," "it turned out that," "we found that," "as it happens." State what happened directly.
- No emdashes. Use periods, commas, semicolons, or parentheses.
