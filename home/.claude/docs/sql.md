...
Formatting and schema conventions for Postgres. The application-layer side (the repository contract, who owns transactions) is in [python.md](python.md).
...
- **Always fully-qualify table names; never alias them.**
  ...
  ```
- **Bind every dynamic value; never assemble SQL from variables.** A query carrying a variable value sends it as a bound parameter, never formatted into the SQL text (no f-string, `%`, `+`, or `.format()`). A fully static query may be a plain string literal.
...
- **The application supplies every column value; the database defaults nothing.** Build the full row in code (`id` as a `uuid4()`, `created_at`/`updated_at`, every field) before the insert, and list every column explicitly in the `INSERT`. Columns carry no `DEFAULT`, so an omitted value surfaces as a `NOT NULL` violation instead of being silently filled. This rules out DB-side generation you'd read back (`DEFAULT gen_random_uuid()`, `SERIAL`/`IDENTITY` via `RETURNING`) and convenience defaults (`DEFAULT now()`, `DEFAULT false`) alike. When a value must come from a sequence, read `nextval(...)` first, then compose and persist the row. When a new `NOT NULL` column needs backfilling on an existing table, populate it inside the migration (add it with a transient `DEFAULT`, then `DROP DEFAULT`; or add it nullable, backfill, then `SET NOT NULL`) so no default survives in the committed schema.
- **Prefer surrogate primary keys.** A table's primary key is an internal `UUID NOT NULL` surrogate with no DB default; foreign keys reference that surrogate, never a natural key, so a natural key can be renamed without re-keying every table pointing at it. A surrogate that exists purely for referential stability is a persistence device and does not surface as a domain attribute. When the domain itself identifies the entity by that id, such as one appearing in URLs, the domain carries it.
